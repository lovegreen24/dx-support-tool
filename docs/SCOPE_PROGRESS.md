...
## 🧮 役員報酬最適化シミュレーションMCP追加(T-013・実施日: 2026-08-28)

**背景**: コンサルタント本人が視聴した外部セミナー動画(税理士・元国税調査官による「旅費規程を活用した節税」講演)を踏まえ、同様の役員報酬シミュレーションを自社ツールに組み込みたいという依頼。参考として提示された既存シミュレーションスクリプト(Pythonで作成されたExcel生成スクリプト複数種)は、所得税を一律20%固定・社会保険料率も年度不明の概算値を使っており、精度が不十分と判断。講演内で実際に画面共有されたシミュレーション画面(スクリーンショット)から実測値を逆算し、正しい計算式(累進課税・給与所得控除・標準報酬月額表)を採用していることを確認した上で、公的資料(国税庁・協会けんぽ・日本年金機構・厚生労働省)を調査し直して実装した。

**実装内容**: `mcp-servers/compensation-optimizer/`(新規MCP、`simulate_executive_compensation`)。
- 役員報酬(月額)と旅費規程による非課税支給(日当・宿泊費等)の組み替えを現行/変更後の2パターンで比較
- 所得税(累進7段階の速算表)・復興特別所得税(2.1%)・住民税(所得割10%+均等割5,000円)・健康保険(協会けんぽ全50等級・都道府県別料率、令和7年度)・厚生年金(全32等級・上限65万円・保険料率18.3%)・雇用保険(令和7年度・一般の事業)を実装
- 永続化なし(コンサルタント本人が都度試算する内部ツールのため、クライアントDBとは独立)
- 健康保険標準報酬月額第39級(79万円)→第30級(50万円)、厚生年金の上限キャップ(65万円)という講演内実例のグレード遷移を単体テストで再現し、等級表データの正しさを検証済み(`tests/unit/socialInsurance.test.ts`)

**意図的なスコープ限定(過剰実装回避)**:
- 配偶者控除・扶養控除・生命保険料控除・小規模企業共済等掛金控除等の個別所得控除は対象外(給与所得控除・社会保険料控除・基礎控除のみで概算)。出力の`notes`に明記し、断定を避ける設計とした(`context/consulting-standards.md` §3-4の断定回避の原則に準拠)
- 給与所得控除は速算式を採用(年収660万円未満は年末調整の号俸表と数千円〜数万円のズレが生じ得る旨をnotesに明記)
- 消費税への影響は具体的な金額を算出せず、定性的な注記(旅費規程支給分は会社側の課税仕入れとして扱われ得るが、実際の納税額への影響は会社全体の課税売上・課税仕入れ状況による)に留めた

**引き継ぎ事項**: 本MCPが参照する税率・保険料率・標準報酬月額表は年度改定があるため、次年度以降に利用する際は`src/rateTables.ts`のコメントに記載した出典URL(国税庁・協会けんぽ・日本年金機構・厚生労働省)で最新版に更新されていないか確認すること。

**動作検証**:
- `npm run build`(tsc)・`npm run test:unit`(Jest)ともに緑。単体テスト32件全てPASS(`tests/unit/incomeTax.test.ts`・`socialInsurance.test.ts`・`simulation.test.ts`)
- 実際のツール呼び出し(福岡・介護保険非該当・現行「月額80万円のみ」→変更後「月額50万円+旅費規程月額30万円」)で動作確認。結果: 年間手取り+1,222,128円、会社負担(事業主分社会保険料)−376,494円、個人の税・社会保険料合計負担−1,222,128円
- 上記の実行結果を比較表として可視化したArtifactを作成し、ユーザーに提示済み(社内提案時の見せ方のたたき台。都度別の金額・都道府県で試す場合は再度ツールを呼び出して数値を差し替える)

**ドキュメント更新箇所**: `CLAUDE.md`(MCP構成にsimulate_executive_compensationを追記)・`docs/requirements.md`(T-013追加、§8ツール数更新)・本ファイル(本セクション)。gitへのコミットは未実施(ユーザーの明示的な指示待ち)。

## 🧮 役員報酬最適化シミュレーションMCPに賞与シミュレーションを追加(T-013拡張・実施日: 2026-08-28)

**背景**: 「旅費規定+賞与年1回のシミュレーション機能を実装してほしい」という依頼。上記T-013の初期実装は意図的に賞与を対象外としていたが、今回その制約を外し、月額報酬・旅費規程支給に加えて年1回の役員賞与を組み込んだ比較ができるよう拡張した。ブランチ`feature/executive-bonus-simulation`で作業。

**実装内容**: `mcp-servers/compensation-optimizer/`を拡張(新規MCPの追加ではなく既存ツールの機能拡張)。
- `CompensationPattern`(現行/変更後の各パターン)に`bonusAnnual`(年間賞与総額)・`bonusPreNotified`(事前確定届出給与としての届出有無)を追加
- 賞与の社会保険料は「標準賞与額」(1,000円未満切り捨て)として月額報酬分とは別枠で算定。健康保険は年度累計573万円、厚生年金は1回150万円の上限を適用(`socialInsurance.ts`に`computeBonusSocialInsurance`を追加)
- 役員賞与が事前確定届出給与の届出要件を満たさない場合、賞与全額が法人税法上の損金に不算入となり会社の実効税率相当のコストが生じる点を試算(`bonusCorporateTaxCost`)。実効税率は任意入力`effective_corporate_tax_rate`とし、損金不算入となるパターンが存在する場合のみ必須とするバリデーションを追加
- 比較結果に`corporateTaxImpactChange`(法人税影響の増減)・`employerTotalCostChange`(事業主分社会保険料+法人税影響の合計コスト増減)を追加

**設計判断の理由**:
- 賞与額は現行・変更後の両方に入力可能とした(既にボーナスを払っている会社が月額⇔賞与の配分を見直すケースにも対応するため。ユーザー承認済み)
- 会社側の法人税への影響(損金算入可否)を試算に含めた(個人の手取りだけでなく会社の総コスト影響を示さないと、賞与への振替提案の妥当性が判断できないため。ユーザー承認済み)
- 実効税率を入力値とした(法人の実効税率は所得水準・自治体により異なり、ハードコードすると誤った試算になるため。都道府県・介護保険該当有無と同様、都度指定する設計に統一)

**動作検証**:
- `npx tsc --noEmit`・`npm run test:unit`(Jest)ともに緑。単体テスト45件全てPASS(既存分含む。賞与の標準賞与額上限・法人税影響コスト・届出済み/未届出の分岐を新規テストで検証)
...
## 🌐 クライアント向け説明ページの作成(役員報酬・固定費削減シミュレーション、実施日: 2026-08-28)

**背景**: T-013(役員報酬最適化シミュレーションMCP)の内容をクライアントにも説明できるようWeb公開してほしいという依頼。当初「本番Webアプリとして機能追加」も検討したが、`compensation-optimizer`はCLAUDE.mdで「local、コンサルタント本人専用の内部ツール」と明記されており、本番Cloud Run/Vercel側にはクライアント向け認証の仕組みが存在しない(単一コンサルタント専用の共有パスワードのみ、`docs/DEPLOYMENT.md`記載の既知の制約)ため、機微な役員報酬・税額試算をそのまま本番環境に公開するのは設計からの逸脱と判断。ユーザーと協議の上、本番インフラ(Cloud Run/Vercel)には一切手を加えず、Claude Artifacts(非公開リンクとして発行し、共有範囲はユーザーが都度選択)でデモページを作成する方針を採用した。

**作成物(いずれもコードリポジトリ外、Claude Artifactsとして公開・URL/共有範囲はユーザー管理)**:
1. **役員報酬シミュレーター**(`https://claude.ai/code/artifact/9ae5fa1e-e832-4c32-936e-1f0c8fa742c8`) — `compensation-optimizer`の計算ロジック(所得税・住民税・社会保険料・賞与の法人税影響)をクライアント側JavaScriptに移植し、現行/変更後パターンを入力すると自動再計算するインタラクティブページ。機能概要・試算の前提条件7項目・今回の機能追加が既存システムに影響しない旨のご案内(振る舞いレベルの説明。プロンプト/コード原文は非開示)を掲載
2. **固定費削減シミュレーション表**(`https://claude.ai/code/artifact/fbcdcb95-1849-42f2-ba9c-3ef26eb69a67`) — 電気代見直しの事例紹介(年間2,700万円→新電力2社比較)に続き、業種別(運送業・倉庫業/製造業/娯楽業/不動産管理・賃貸業/医療法人/社会福祉法人/宿泊業/学校法人/流通業の9業種)の固定費削減シミュレーター。業種タブを切り替えると各業種4項目(複合機・電気料金・エレベーター保守・レンタルマット等、計36データポイント)が並び、項目ごとの選択トグルと現行費用の入力欄で、実績データに基づく削減率を用いたリアルタイム試算・年換算の合計削減額サマリーを表示

**データ出典**: 業種別削減率・削減額は、ユーザー提供のセミナー資料(`税務財務経営戦略講座`フォルダ、スクリーンショット28枚+同一内容のzip)内の「削減実績」スライドを実際に読み取って収集した実測値(9業種分)。zipの中身はスクリーンショットと完全同一で、zip固有の追加情報は無かった。

**設計判断**:
- 当初は役員報酬シミュレーターのページに固定費削減の内容も統合したが、ユーザーから「役員報酬シミュレーションとは別の形で新たに作ってほしい」との指摘を受け、URLも別の独立したArtifact「固定費削減シミュレーション表」に分割し、役員報酬シミュレーターのページは元の内容に復元した
- 固定費削減シミュレーション表は当初は静的な実績表として作成したが、ユーザーから「業種ごとに項目を選択ボタンでそれぞれ数字を入れられるように」との追加要望を受け、静的表から対話型シミュレーター(選択トグル+現行費用の入力欄+リアルタイム計算+年換算合計サマリー)に作り直した
- 削減率は実績データに基づく目安値であり同水準の削減を保証しない旨を、両ページの注記に明記(過大な期待を避けるため)
